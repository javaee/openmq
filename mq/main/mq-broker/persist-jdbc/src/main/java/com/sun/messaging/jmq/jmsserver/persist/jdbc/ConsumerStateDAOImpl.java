...
class ConsumerStateDAOImpl extends BaseDAOImpl implements ConsumerStateDAO {
    ...
    public void updateTransaction( Connection conn, SysMessageID sysMsgID,
        ...
        try {
            ...
            if ( existingTxnID > 0 ) {
                TransactionUID existingTID = new TransactionUID(existingTxnID);
                TransactionState ts = dbMgr.getDAOFactory().getTransactionDAO().
                                          getTransactionState( conn, existingTID );
                if (ts.getState() == TransactionState.ROLLEDBACK) {
                    Object[] args = { "["+sysMsgID+", "+conUID+"]", 
                                      String.valueOf(existingTxnID), txnUID };
                    logger.log(Logger.INFO, br.getKString(
                        BrokerResources.I_CLEAR_ROLLEDBACK_TXN_ACK_FOR_NEW_TXN, args));
                    clearTransaction( conn, existingTID );
                } else {
            ...
            pstmt = dbMgr.createPreparedStatement( conn, updateTransactionSQL );
            ...
            if ( pstmt.executeUpdate() == 0 ) {
                ...
                // Check if ack exists
                existingTxnID = getTransaction( conn, sysMsgID, conUID );
                if ( existingTxnID > 0 ) {
                    String ack = "["+sysMsgID+", "+conUID+"]";
                    throw new TransactionAckExistException(
