...
implements GrizzlyService, NotificationInfo
{
    ...
    private static final String DEFAULT_GRIZZLY_HTTP_SERVER_NETWORKLISTENER_NAME = "grizzly";
    ...
    public WebSocketIPService(String name, int type, PacketRouter router,
        ...
        String key =  WebSocketIPServiceFactory.SERVICE_PREFIX +name+".services";
        ...
        try {
            ...
            if (hostn != null) {
                httpServer = HttpServer.createSimpleServer(docroot, hostn, prange);
            } else {
                httpServer = HttpServer.createSimpleServer(docroot, prange);
            }
            networkListener = httpServer.getListener(
                DEFAULT_GRIZZLY_HTTP_SERVER_NETWORKLISTENER_NAME);
