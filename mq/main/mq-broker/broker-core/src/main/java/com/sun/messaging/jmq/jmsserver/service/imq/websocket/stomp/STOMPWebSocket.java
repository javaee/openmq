...
public class STOMPWebSocket extends MQWebSocket implements StompOutputHandler {
    ...
    protected void writePacket(Packet pkt) throws IOException {
        String[] args = { "STOMPWebSocket.writPacket(): Unexpected call" };
    ...
    protected void processData(String text) throws Exception {
        if (DEBUG) {
            logger.log(Logger.INFO, Thread.currentThread()+getLogString()+" processData:text="+text);
        }
	...
	if (ex != null) {
		...
		message = (StompFrameMessageImpl)((StompFrameParseException)ex).
                          getStompMessageERROR(StompFrameMessageImpl.getFactory());
            } else {
    ...
    protected void sendFatalError(Exception e) {
        ...
        try {
            StompFrameMessage err = stompProtocolHandler.
                toStompErrorMessage("STOMPWebSocket", e);
            err.setFatalERROR();
    ...
    public void sendToClient(final StompFrameMessage msg,
        ...
        if (DEBUG) {
            logger.log(logger.INFO, 
            "STOMPWebSocket@"+hashCode()+" sendToClient("+msg+")");
        }
	...
	try {
		...
		if (msg.isFatalERROR()) {
            ...
            synchronized(closeLock) {
                if (isClosed()) {
                    logger.log(logger.INFO, "STOMPWebSocket@"+this.hashCode()+" closed");
                    return;
                }
                Buffer buf = (Buffer)msg.marshall(memManager).getWrapped();
                byte[] bb = new byte[buf.remaining()]; //XXopt
                buf.get(bb);
                send(bb);
                if (DEBUG) {
                    logger.log(logger.INFO, 
                    "STOMPWebSocket@"+hashCode()+ " SENT "+bb.length+" bytes");
                }
            }
	} catch (Exception e) {
