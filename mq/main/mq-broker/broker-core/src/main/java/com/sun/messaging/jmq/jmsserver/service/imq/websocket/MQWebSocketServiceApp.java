...
public class MQWebSocketServiceApp extends WebSocketApplication {
    ...
    throws Exception {
        ...
        if (!myurl.getProtocol().equals(ou.getProtocol())) { 
            return false;
        }
        if (myurl.getHost().equals(ou.getHost())) {
            return true;
        }
        InetAddress oia = InetAddress.getByName(ou.getHost()); 
        if (InetAddress.getByName(myurl.getHost()).equals(oia)) {
            return true;
        }
        ...
        while (itr1.hasNext()) {
            url = itr1.next();
            if (!url.getProtocol().equals(ou.getProtocol())) {
                continue;
            }
