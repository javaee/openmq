 ...
 */
public class GrizzlyIPService extends IMQService implements NotificationInfo
{
    ...
    private static final SSLEngineConfigurator initializeSSL(
        String servicen, GrizzlyProtocolImpl proto)
        throws Exception {

        Globals.getLogger().log(Logger.INFO,  Globals.getBrokerResources().getKString(
                                BrokerResources.I_INIT_FOR_SERVICE, proto.getType(), servicen));

        Properties sslprops = KeystoreUtil.getDefaultSSLContextConfig("GrizzlyIPService", null);
